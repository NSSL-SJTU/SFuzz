...
with open("%s/extrace_result/%s.simresult"%(os.getcwd(), get_input_file_path().split('/')[-1]), 'w') as f:
    # print(get_named_func())
    json.dump(get_named_func(), f,indent=2)
    print("IDA extraction result has been saved in %s/extrace_result/%s.simresult"%((os.getcwd(), get_input_file_path().split('/')[-1])))
